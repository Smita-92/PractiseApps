...
public class excelutility {
	...
	public void setDataIntoExcel(String sheetName, int rowNum, int cellNum,String data) throws Throwable, IOException {
		FileInputStream fis = new FileInputStream("./Amazon/mobiles.xlsx");
		Workbook wb = WorkbookFactory.create(fis);
		wb.getSheet(sheetName).getRow(rowNum).createCell(cellNum, CellType.STRING).setCellValue(data);
		FileOutputStream fos = new FileOutputStream("./Amazon/mobiles.xlsx");
		wb.write(fos);
		wb.close();
		fos.close();
	}
